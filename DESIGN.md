# Design System — Claude.com Editorial (Cream + Coral + Dark Navy)

> このファイルはアプリ全体のデザインの単一ソース・オブ・トゥルースです。
> 実装は `frontend/src/theme/tokens.ts` と `frontend/src/theme/global.css` にここの値を反映しています。
> トークンを変更したら、その2ファイルも更新してください。
...
> 本アプリの実装: 見出し=**EB Garamond**（weight 400・負トラッキング）、本文=**Inter**、コード=**JetBrains Mono**。すべて `@fontsource` でローカル同梱（CDN不使用＝完全オフライン）。
